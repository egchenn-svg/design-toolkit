  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h2 className="text-xl font-bold text-gray-900 mb-5">添加自定义工具</h2>
              ...
              {emojiOptions.map(e => (
                <button
                  ...
                  className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-all ${
                    icon === e ? 'bg-violet-100 ring-2 ring-violet-400' : 'bg-gray-100 hover:bg-gray-200'
                  }`}
                >
                  {e}
                </button>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  mode === 'jump' ? 'bg-violet-500 text-white' : 'bg-gray-100 text-gray-600'
                }`}
              >
                🔗 新窗口打开
              </button>
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  mode === 'iframe' ? 'bg-violet-500 text-white' : 'bg-gray-100 text-gray-600'
                }`}
              >
                📋 页内嵌入
              </button>
      ...
      <div>
        <div className="flex items-center gap-3 mb-6">
          <button
            ...
            className="px-4 py-2 bg-white text-gray-600 rounded-lg hover:bg-gray-100 transition-colors font-medium border border-gray-200"
          >
            ← 返回
          </button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">我的工具集</h1>
          <p className="text-gray-500">自定义常用工具，快速访问</p>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          className="px-5 py-2.5 bg-violet-500 text-white rounded-xl hover:bg-violet-600 transition-colors font-medium"
        >
          + 添加工具
        </button>
      </div>
      ...
      {tools.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
          ...
          <button
            onClick={() => setShowAdd(true)}
            className="px-6 py-2.5 bg-violet-500 text-white rounded-xl hover:bg-violet-600 transition-colors font-medium"
          >
            添加第一个工具
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {tools.map(tool => (
            <div
              key={tool.id}
              className="relative group bg-white border border-gray-200 rounded-2xl p-5 hover:border-gray-300 transition-all duration-300 cursor-pointer hover:shadow-sm"
            ...
            >
              <button
                ...
                className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-red-50 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
              >
                ×
              </button>
              ...
              <div className="mt-2">
                <span className={`px-2 py-0.5 text-xs rounded-md ${
                  tool.mode === 'iframe' ? 'bg-violet-50 text-violet-600' : 'bg-gray-50 text-gray-500'
                }`}>
